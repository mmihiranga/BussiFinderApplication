...
const MultipleResultP = () => {
    ...
    const downloadReport = (i) => {
        var j = i;
        const doc = new jsPDF();
                    doc.setFontSize(22);
                    doc.setFont("courier", "bolditalic");
                    doc.text("Pharmacy Success Percentage Prediction", 105, 8, null, null, "center");
                    doc.setFont("helvetica", "normal");
                    doc.setFontSize(16);
                    doc.text(`Type : ${businessDetails.value[j].type}`, 20, 30);
                    doc.setFont("helvetica", "normal");
                    doc.setFontSize(16);
                    doc.text(`Type : ${businessDetails.value[j].type}`, 20, 30);
                    doc.text(`Latitude : ${businessDetails.value[j].latitude}`, 20, 40);
                    doc.text(`Longitude : ${businessDetails.value[j].longitude}`, 20, 50);
                    doc.text(`Available medical places count : ${businessDetails.value[j].locationFeatures.medicalPlacesCount}`, 20, 60);
                    doc.text(`Distance to nearest bus station : ${businessDetails.value[j].locationFeatures.busStationDistance}`, 20, 70);
                    doc.text(`24 hours opening : ${businessDetails.value[j].locationFeatures.OpenHours}`, 20, 80);
                    doc.text(`Availability of delivery service : ${businessDetails.value[j].locationFeatures.deliver}`, 20, 90);
                    doc.text(`Wheel chair accessible entrance : ${businessDetails.value[j].locationFeatures.WhlChairEntrance}`, 20, 100);
                    doc.text(`Wheel chair accessible car park : ${businessDetails.value[j].locationFeatures.WhlChairPark}`, 20, 110); 
                    doc.text(`Cash payment : ${businessDetails.value[j].locationFeatures.cashPay}`, 20, 120);
                    doc.text(`Card payment : ${businessDetails.value[j].locationFeatures.cardPay}`, 20, 130);
                    doc.setFont("helvetica", "bold");
                    doc.text(`Final Prediction : ${businessDetails.value[j].ml_result}%`, 20, 140);
                    doc.save("Report.pdf"); 
            }
                ...
                >
                    ...
                    <Box
                        ...
                        <Box className={classes.featuresBoxFlex}>
                            <div className={classes.featureTitleTxt}>Pharmacy Location Based Features</div>
                    ...
                    <Box className={classes.featuresBoxFlex}>
                        <div className={classes.featureTitleTxt}>Pharmacy Service Based Features</div>
                        <Box className={classes.featuresBox}>
                            <Grow in={checked} style={{ transformOrigin: '0 0 0' }} {...(checked ? { timeout: 1100 } : {})} >
                ...
                >
                    ...
                    <Box
                        ...
                        <Box className={classes.featuresBoxFlex}>
                            ...
                            <Box className={classes.featuresBox}>
                                <Grow in={checked} style={{ transformOrigin: '0 0 0' }} {...(checked ? { timeout: 500 } : {})} >
                                    <div className={classes.features}><GiPathDistance style={{ color: '#df5e52',fontSize: '20px' }} />Distance to bus station  - {businessDetails.value[i].locationFeatures.busStationDistance}m</div>
                                </Grow>
                                <Grow in={checked} style={{ transformOrigin: '0 0 0' }} {...(checked ? { timeout: 800 } : {})} >
                                    <div className={classes.features}><FaClinicMedical style={{ color: '#2f9f48',fontSize: '20px' }} />Medical Places Count - {businessDetails.value[i].locationFeatures.medicalPlacesCount}</div>
                                </Grow>
                            </Box>
                        </Box>
                    </Box>
                    <Box className={classes.featuresBoxFlex}>
                        ...
                        <Box className={classes.featuresBox}>
                            <Grow in={checked} style={{ transformOrigin: '0 0 0' }} {...(checked ? { timeout: 1100 } : {})} >
                                <div className={classes.features}><AiOutlineClockCircle style={{ color: '#cbe075', fontSize: '20px', marginInline: '8px' }} />24 hours opening : {businessDetails.value[i].locationFeatures.OpenHours} </div>
                            </Grow>
                            <Grow in={checked} style={{ transformOrigin: '0 0 0' }} {...(checked ? { timeout: 1100 } : {})} >
                                <div className={classes.features}><FaRegCreditCard style={{ color: '#f065aa', fontSize: '20px', marginInline: '8px' }} />Card payment : {businessDetails.value[i].locationFeatures.cardPay}   </div>
                            </Grow>
                            <Grow in={checked} style={{ transformOrigin: '0 0 0' }} {...(checked ? { timeout: 1100 } : {})} >
                                <div className={classes.features}><BsCashCoin style={{ color: '#2f8eff', fontSize: '20px', marginInline: '8px' }} />Cash payment : {businessDetails.value[i].locationFeatures.cashPay}   </div>
                            </Grow>
                            <Grow in={checked} style={{ transformOrigin: '0 0 0' }} {...(checked ? { timeout: 1100 } : {})} >
                                <div className={classes.features}><FaWheelchair style={{ color: '#1d7e20', fontSize: '20px', marginInline: '8px' }} />Wheel chair entrance : {businessDetails.value[i].locationFeatures.WhlChairEntrance}   </div>
                            </Grow>
                            <Grow in={checked} style={{ transformOrigin: '0 0 0' }} {...(checked ? { timeout: 1100 } : {})} >
                                <div className={classes.features}><GrWheelchairActive style={{ color: '##9370db', fontSize: '20px', marginInline: '8px' }} />Wheel chair car park : {businessDetails.value[i].locationFeatures.WhlChairPark}   </div>
                            </Grow>
                            <Grow in={checked} style={{ transformOrigin: '0 0 0' }} {...(checked ? { timeout: 1100 } : {})} >
                                <div className={classes.features}> <MdOutlineDeliveryDining style={{ color: '#ff802f', fontSize: '22px', marginInline: '8px' }} />Delivery : {businessDetails.value[i].locationFeatures.deliver}  </div>
                            </Grow>
                        </Box>
                    </Box>
                    <Box
                    ...
                    >
                        <DownloadBtn onClick={() => downloadReport(i)}>Download Report <SystemUpdateAltRoundedIcon /></DownloadBtn>
                    </Box>
...
export default MultipleResultP