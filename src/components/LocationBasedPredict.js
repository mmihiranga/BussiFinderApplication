...
import { updateUser, userInfo } from "../features/user";
import api from "../api";
...
const LocationBasedPredict = () => {
  ...
  const userInfoDetails = useSelector((state) => state.user.userDetails);
  ...
  const handleNext = () => {
    if (userInfoDetails.locationPredictCount <= 0) {
      setErrorMsg("You have no credit left to predict location");
      setError(true);
    } else if (
      ...
      API.post("hotel/transportationmodes", body, headers)
        .then(function (transportModesResult) {
          API.post("hotel/attractionplaces", body, headers)
            .then(function (attractionplacesResult) {
              API.post("hotel/nearbyhotel", body, headers)
                .then(function (nearByHotelResult) {
                  ...
                  let transportationModesCount =
                    transportModesResult.data.transportationmodes_count;
                  let attractionPlacesCount =
                    attractionplacesResult.data.attractionplaces_count;
                  let nearByHotelReviewCount =
                    nearByHotelResult.data.rating_count;
                  ...
                  console.log(
                    ...
                    competitors,
                    businessDetails.value[0].serviceDetails
                  );
                  let mlReqBody = {
                  ...
                  API.post("restaurant/BusinessCount", body, headers)
                    .then(function (otherBusinessCount) {
                      ...
                      axios
                        .post(
                          "https://businesssuccesspredictor.herokuapp.com/hotelLocationBase",
                          mlReqBody
                        )
  ...
  const handleRestaurantNext = (
    ...
    API.post("restaurant/Competitors", body, headers)
      .then(function (competitors) {
        API.post("restaurant/Education", body, headers)
              ...
              .then(function (workPlaces) {
                API.post("restaurant/ShoppingMallsCount", body, headers)
                  .then(function (response) {
                    console.log(
                      "ShoppingMallsCount",
                      response.data.totalShoppingMallsCount
                    );
                    API.post("restaurant/DistanceToCity", body, headers)
                      .then(function (res) {
                        let distance = res.data.split(" ");
                        let Competitors =
                          competitors.data.totalRestaurantsCount;
                        let EducationRelatedPlacesCount =
                          education.data.totalEducationRelatedPlacesCount;
                        let WorkPlacesCount =
                          workPlaces.data.totalWorkPlacesCount;
                        ...
                        API.post("restaurant/BusinessCount", body, headers)
                          .then(function (otherBusinessCount) {
                            ...
                            axios
                              .post(
                                "https://businesssuccesspredictor.herokuapp.com/restaurantLocationBase",
                                mlReqBody
                              )
                              .then(function (ML_Result) {
                                ...
                                dispatch(
                                  addBusiness({
                                    ...
                                    HotelFeatures: HotellocationFeatures,
                                    HotelbusinessCount: HotelBusinessCount,
                                    Hotelml_result: HotelML_Result,
                                    RestaurantFeatures: locationFeatures,
                                    RestaurantCount: BusinessCount,
                                    Restaurant_ml_result: ML_Result.data.data,
                                  })
                                );
                                setIsResLoading(false);
                                changeDB(
                                  userInfoDetails._id,
                                  userInfoDetails.locationPredictCount - 1
                                );
  ...
  const changeDB = (uid, LocationPredictCount) => {
    const body = {
      id: uid,
      locationPredictCount: LocationPredictCount,
    };
    console.log(body);
    api
      .put(`/user/updateUser/`, body)
      .then((res) => {
  ...
  return (
    ...
    >
      ...
      {isHotelLoading == false && isResLoading == false ? (
        <ThemeProvider theme={theme}>
          <Container
            component="main"
            maxWidth={"md"}
            sx={{ p: 8, boxSizing: "border-box" }}
          >
            <Paper
              variant="outlined"
              sx={{
